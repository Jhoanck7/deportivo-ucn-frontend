...
export class AdminDashboardComponent implements OnInit {
    ...
    this.http.get<RamaDeportiva[]>(this.nominasApiUrl)
      .subscribe({
        ...
        error: (err) => {
          ...
          // Datos mock idénticos al de la captura de pantalla de Deportes UCN
  ...
  editRama(rama: RamaDeportiva): void {
    const nuevoDt = prompt(`Editar Director Técnico para ${rama.nombre}:`, rama.dtAsignado);
    if (nuevoDt !== null && nuevoDt.trim() !== '') {
      rama.dtAsignado = nuevoDt;
      alert(`DT de ${rama.nombre} actualizado a: ${nuevoDt}`);
    }
  }
  ...
  onTimeTabChange(tab: string): void {
    ...
    // Aquí se podrían filtrar reservas según el tab temporal si se desea
  }
  ...
  contactUser(reserva: Reserva): void {
    ...
    // Cambiar estado a contactado localmente para feedback inmediato
